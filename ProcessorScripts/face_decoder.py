# This file is taken from here: https://github.com/microsoft/Deep3DFaceReconstruction/blob/master/face_decoder.py
...
class BFM():
    def __init__(self, model_path='BFM/BFM_model_front.mat'):
        ...
        self.point_buf = tf.constant(
            model['point_buf'])  # triangle indices for each vertex that lies in. starts from 1. [N,8]
        self.face_buf = tf.constant(model['tri'])  # vertex indices in each triangle. starts from 1. [F,3]
        self.keypoints = tf.squeeze(
            tf.constant(model['keypoints']))  # vertex indices of 68 facial landmarks. starts from 1. [68,1]
...
class Face3D():
    ...
    def Projection_block(self, face_shape, focal=1015.0, half_image_width=112.):
        ...
        p_matrix = tf.concat(
            [focal * tf.ones([batchsize, 1]), tf.zeros([batchsize, 1]), half_image_width * tf.ones([batchsize, 1]),
             tf.zeros([batchsize, 1]), \
             focal * tf.ones([batchsize, 1]), half_image_width * tf.ones([batchsize, 1]), tf.zeros([batchsize, 2]),
             tf.ones([batchsize, 1])], axis=1)
